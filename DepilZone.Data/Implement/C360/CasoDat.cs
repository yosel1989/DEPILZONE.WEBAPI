	...
	public class CasoDat : ICasoDat
	{
        public async Task<List<CasoDTO>> Listar()
        {
            try
            {
                ...
                using SqlCommand cmd = new SqlCommand("C360_SP_Caso_Listar", conn)
                {
        ...
        public async Task<List<CasoDTO>> ListarByEstado(int idEstado)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("C360_SP_Caso_ListarByEstado", conn)
                {
                    CommandType = System.Data.CommandType.StoredProcedure
                };
        ...
        public async Task<bool> Registrar(CasoDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("C360_SP_Caso_Registrar", conn)
                {
                    CommandType = System.Data.CommandType.StoredProcedure
                };
        ...
        public async Task<bool> Modificar(int id, CasoDTO model)
        {
            try
            {
                using SqlConnection conn = DBConn.ConexionSQL();
                await conn.OpenAsync();
                using SqlCommand cmd = new SqlCommand("C360_SP_Caso_Modificar", conn)
                {
                    CommandType = System.Data.CommandType.StoredProcedure
                };
        ...
        static async Task<List<CasoDTO>> ReadListar(DbDataReader reader)
        {
            try
            {
                List<CasoDTO> collection = new List<CasoDTO>();
                while (await reader.ReadAsync())
                {
                    CasoDTO obj = new CasoDTO();
